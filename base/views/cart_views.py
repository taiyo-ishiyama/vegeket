...
class CartListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        for item_pk, quantity in cart['items'].items():
            ...
            obj.subtotal = round(float(obj.price * quantity), 3)
            self.queryset.append(obj)
            self.total += round(float(obj.subtotal), 2)
        self.tax_included_total = round(float(self.total * (settings.TAX_RATE + 1)), 2)
